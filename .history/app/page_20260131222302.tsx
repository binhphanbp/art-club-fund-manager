    ...
    <main className="min-h-screen mesh-gradient overflow-hidden">
        ...
        >
          {leadership.map((leader, index) => (
            <motion.div
              ...
              variants={cardVariants}
              whileHover={{ y: -10 }}
              className={`relative group ${index === 0 ? "md:scale-110 md:z-10" : ""}`}
            >
              {/* Special glow for president */}
              {index === 0 && (
                <div className="absolute -inset-1 bg-gradient-to-r from-yellow-400 via-amber-500 to-orange-500 rounded-2xl blur-lg opacity-40 group-hover:opacity-60 transition-opacity" />
              )}
              
              <div className={`relative bg-card border ${index === 0 ? "border-yellow-500/50" : "border-border"} rounded-2xl p-6 text-center overflow-hidden`}>
                {/* Crown for president */}
                {index === 0 && (
                ...
                {/* Avatar */}
                <div className={`w-24 h-24 mx-auto mb-4 rounded-full bg-gradient-to-br ${leader.gradient} p-1 ${index === 0 ? "mt-4" : ""}`}>
                  <div className="w-full h-full rounded-full bg-card flex items-center justify-center">
                    <leader.icon className="w-10 h-10 text-purple-400" />
                  </div>
                </div>

                {/* Info */}
                <h3 className={`text-xl font-bold mb-1 ${index === 0 ? "text-yellow-400" : "text-white"}`}>
                  {leader.name}
                </h3>
                <p className={`text-sm font-medium mb-3 ${index === 0 ? "text-amber-400" : "text-purple-400"}`}>
                  {leader.role}
                </p>
                ...
                {/* Decorative */}
                <div className={`absolute -bottom-4 -right-4 w-24 h-24 bg-gradient-to-br ${leader.gradient} opacity-20 rounded-full blur-2xl`} />
              </div>
            </motion.div>
          ))}
        </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {departmentLeaders.map((leader) => (
              <motion.div
                ...
                className="bg-card border border-border rounded-xl p-5 text-center card-glow"
              >
                <div className={`w-16 h-16 mx-auto mb-3 rounded-full bg-gradient-to-br ${leader.gradient} p-0.5`}>
                  <div className="w-full h-full rounded-full bg-card flex items-center justify-center">
                    <Star className="w-7 h-7 text-white" />
                  </div>
                </div>
                <h4 className="font-semibold text-white">{leader.name}</h4>
                <p className={`text-sm bg-gradient-to-r ${leader.gradient} bg-clip-text text-transparent font-medium`}>
                  {leader.role}
                </p>
              </motion.div>
            ))}
      ...
      <section id="members" className="py-20 px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-gray-400 text-lg max-w-xl mx-auto">
            Những nghệ sĩ tài năng đang tỏa sáng
          </p>
        </motion.div>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8">
        <motion.div
          ...
          viewport={{ once: true }}
          className="max-w-4xl mx-auto text-center bg-gradient-to-r from-purple-900/50 to-pink-900/50 rounded-3xl p-10 md:p-16 border border-purple-500/20 relative overflow-hidden"
        >
          ...
          <div className="relative z-10">
            ...
            <p className="text-gray-300 text-lg mb-8 max-w-xl mx-auto">
              Đăng nhập để quản lý đóng góp quỹ và theo dõi hoạt động CLB
            </p>
            <Link href="/login">
              <Button
                size="lg"
                className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white px-10 py-6 text-lg rounded-full shadow-lg shadow-purple-500/25"
              >
                Đăng nhập ngay
