...
async function checkSuperAdminAccess() {
  ...
  if (!dbUser || dbUser.role !== 'SUPER_ADMIN') {
    return { success: false, error: 'Chỉ Super Admin mới có quyền thực hiện thao tác này' };
  }

  return { success: true, user: dbUser };
}

// Close the month: lock all contributions + create MonthlyClose record
export async function closeMonth(month: number, year: number) {
  try {
    ...
    if (existing) {
      return { success: false, error: `Tháng ${month}/${year} đã được chốt sổ rồi.` };
    }
    ...
    // Total approved amount for that month
    ...
    const totalAmount = monthlyApproved._sum.amount ?? 0;

    // Lock all contributions in this month (approved & pending)
    ...
    // Create MonthlyClose record
    await prisma.monthlyClose.create({
      data: {
        month,
        year,
        totalAmount,
        closedBy: accessCheck.user!.fullName,
      },
    });
    ...
    return {
      success: true,
      message: `Đã chốt sổ tháng ${month}/${year}. Tổng thu: ${totalAmount.toLocaleString('vi-VN')}đ`,
      totalAmount,
    };
  } catch (error: any) {
